...
from core.entity.po.NovelEntity import NovelEntity, SceneEntity
...
class NovelMapper(NovelMapperInterface):
    ...
    def get_novel_by_id(self, novel_id: int) -> ResponseAllNovelDto:
        ...
        try:
            chapters = [ResponseAllChapterDto(
                ...
                scene=[
                    ResponseSceneDto(scene_id=scene.scene_id,
                                     scene_name=scene.scene_name,
                                     scene_desc=scene.scene_desc,
                                     create_time=scene.create_time,
                                     parent=scene.parent,
                                     chapter=scene.chapter.chapter_id)
                    for scene in SceneEntity.select(lambda data: data.chapter.chapter_id == chapter.chapter_id)[:]
